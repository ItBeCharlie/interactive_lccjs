...
class Interpreter {
  ...
  run(listing) {
    ...
    let paneLayout = { column0: "rcmo", column1: "", column2: "" };
    let columnCount = 1;
    let codeLines = 5;
    let colors = {
      old: "\x1b[91m",
      ...
      if (this.options.interactiveMode) {
        ...
        switch (input.inputLine[0]) {
          ...
          case "c": // Set code window lines
            output = this.handleCodeWindowLines(input.inputLine.substring(1));
            if (output.error == "") {
          ...
          case "l": // Set pane layout
            output = this.handlePaneLayout(input.inputLine.substring(1));
            if (output.error == "") {
  ...
  displayHelpMenu() {
    ...
    infoPrompt += "s{register}: Stack Controller Follow Mode (default fp)\n";
    ...
    infoPrompt += "l{'r,c,m,o,/'} Layout Editor (default rcmo) \n";
  ...
  executeNextInstruction(readInNewInput) {
    ...
    if (this.debugMode && this.running) {
      ...
      if (this.hasJumped) {
        ...
        regsOrFlagsOutput += `<pc = ${prevPC.toString(16)}/${this.pc.toString(
          16
        )}>`;
  ...
  hexToMnemonic(hex) {
    ...
    if (mnemonic === "CASE10") {
      ...
      mnemonic =
        extendedMnemonics[hex & 0x000f] || `Unknown(${hex.toString(16)})`;
    }
